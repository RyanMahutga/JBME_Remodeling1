...
% non-percolating elastin network
[npE_fibers] = interiorDelaunay(nodesE);
%[npE_fibers,deg_npE] = NetworkPare(npE_fibers, num_nodes,12 );
...
%     A_fibers=[];
%     
%     num_keep = round(length(nodesE)/keep_sets(n));
%     idx_keep=datasample(1:length(nodesE),num_keep,'Replace',false); % randomly select nodes to connect to cell
%     for f=1:length(idx_keep)
%         keep_nodes(f,:) = nodesE(idx_keep(f),:); % kept nodes
%     end
%     
%     temp_nodes=[nodesA;keep_nodes]; % temporary nodes
%     
%     [A_fibers] = periodicDelaunay(temp_nodes);
%     [A_fibers] = removeDupes( A_fibers );
%     
%     for m = 1:length(A_fibers)
%         if A_fibers(m,1) > numAnodes
%             A_fibers(m,1) = idx_keep(A_fibers(m,1) - numAnodes);
%         else
%             A_fibers(m,1) = numEnodes + A_fibers(m,1);
%         end
%         if A_fibers(m,2) > numAnodes
%             A_fibers(m,2) = idx_keep(A_fibers(m,2) - numAnodes);
%         else
%             A_fibers(m,2) = numEnodes + A_fibers(m,2);
%         end
%     end
%     
%     fibs{n} = A_fibers;
...
id1 = {'A50C_','A100C_','A0C_'}; % indentifiers for percent connected ECM nodes to actin
id2 = {'pE_pC_','pE_npC_'}; %,'pE_npC_','npE_npC_'}; % indetifiers for percolating and nonpercolating networks of collagen and elastin
...
for k=1:3
    ...
    if k==3
        fibers1 = uA_fibers;
        stab_nodes = [indexstabE, indexstabA,-1];
    else
        fibers1 = fibs{k};
    ...
    for c=1:2
        fibers2=[];
        if c==1 % both percolating
            fibers2 = [pE_fibers; pC_fibers];
            len2 = length(pE_fibers);
            len3 = length(pC_fibers);
        elseif c==3% collagen percolating, elastin not percolating
            fibers2 = [npE_fibers ; pC_fibers];
            len2 = length(npE_fibers);
            len3 = length(pC_fibers);
        elseif c==2 % elastin percolating, collagen not percolating
            fibers2 = [pE_fibers ; npC_fibers];
            len2 = length(pE_fibers);
            len3 = length(npC_fibers);
        else
            fibers2 =[npE_fibers ; npC_fibers];
            len2 = length(npE_fibers);
            len3 = length(npC_fibers);
        end
...
%             end
            ...
            if k==3
               plot3(nodes(stab_nodes(2),1), nodes(stab_nodes(2),2),nodes(stab_nodes(2),3),'bo','LineWidth',3,'MarkerSize',7) 
            end
