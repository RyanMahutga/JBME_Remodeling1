...
void networkSolver(int num_nodes, int num_fibers, double x_scale, Matrix3d F, network& network0, 
	network& network_n, fiberResults& fiberResults_n, netResults& netResults_n, bool guess, VectorXi stab_node)
{
	...
	int iter_max = 100; // maximum Newton Iterations 
	...
	else // solve via newton
	{
		for (int iter = 0; iter < iter_max; iter++) // Newton's Method
		{
			...
			max_err = residual.maxCoeff(); // maximum error
			min_err = residual.minCoeff();
			...
			JacobiSVD<MatrixXd> svd(Jac); // calculating Jacobian condition number
			...
			*/
			...
			if (num_nodes < 0)
			{
				...
				//DGMRES<SparseMatrix<double>, Lower | Upper, DiagonalPreconditioner<double>>  solver;
	...
	// Calculating volume averaged stress
	calcNetStress(nodes_n, fibers_n, network_n, fiberResults_n, netResults_n, F, num_fibers, x_scale);

	netResults_n.Jacobian = MatrixXd(J);
	
	if (flag == 1)
	{
		cout << "Failed to Converge!" << endl << "Norm of Residuals: " << err << endl << "Maximum Force: " << fiberResults_n.fib_forces.maxCoeff() << endl << endl;
	}
