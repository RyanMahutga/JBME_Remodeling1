...
void updateCrossing(MatrixXd& nodes_mapped, MatrixXd& nodes_n, MatrixXd& fibers_n, const int num_nodes, const int num_fibers, const Matrix3d F)
{
	// defining local values
	double marg = 1e-12; // margin of error for finding bounds
	...
	int q = 0; int p = 0; int k = 0; int h = 0;

	double current_node1; // current nodal coordinates of static node
	double current_node2; // current nodal coordinates of dynamic node
	...
	for (int n = 0; n < num_fibers; n++)
	{
		...
		{
			current_node1 = nodes2(node1, i); // static nodes
				...
				current_node1 < lo_bnd || current_node2 < lo_bnd) // out of lower bound
			{
				...
				cout << current_node1 << endl << endl;
				cout << "NODE#: " << node2 << endl;
				cout << current_node2 << endl << endl;*/
	...
	//cout << "FIBERS: " << endl << fibers_n << endl << endl;

	Matrix3d T(3, 3); // translation matrix
	Matrix3d I(3, 3); I << 1, 0, 0, 0, 1, 0, 0, 0, 1;

	T = 0.5*(F - I); // rigid translation to keep corner at -0.5*(1 1 1)
	for (int n = 0; n < num_nodes; n++)
	{
		// update nodal coordinates in deformed geometry
		nodes_n(n, 0) = F(0, 0)*(nodes_mapped(n, 0)) + F(0, 1)*(nodes_mapped(n, 1)) + F(0, 2)*(nodes_mapped(n, 2))
			+ (T(0, 0) + T(0, 1) + T(0, 2)); // deformed nodal coordinate x
		nodes_n(n, 1) = F(1, 0)*(nodes_mapped(n, 0)) + F(1, 1)*(nodes_mapped(n, 1)) + F(1, 2)*(nodes_mapped(n, 2))
			+ (T(1, 0) + T(1, 1) + T(1, 2)); // deformed nodal coordinate y
		nodes_n(n, 2) = F(2, 0)*(nodes_mapped(n, 0)) + F(2, 1)*(nodes_mapped(n, 1)) + F(2, 2)*(nodes_mapped(n, 2))
			+ (T(2, 0) + T(2, 1) + T(2, 2)); // deformed nodal coordinate z	
	}
}
