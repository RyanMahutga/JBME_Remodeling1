...
{
	double dlam = 1e-6, mag_err = 100, err_thresh = 1;
	Matrix3d dF;
	...
	for (int n = 0; n < 100; n++)
	{
		...
		mag_err = init_err(0)*init_err(0) + init_err(1)*init_err(1);
		old_err(0) = -1.0*init_err(0);
		old_err(1) = -1.0*init_err(1);
