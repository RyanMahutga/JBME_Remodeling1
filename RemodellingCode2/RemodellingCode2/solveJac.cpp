...
{
	double R0 = 400e-6, H0 = 150e-6, mu = 0.003; // aorta radius and wall thickness unloaded and viscosity (Pa/s) and flow rate (m^3/s)

	WSS = 4 * mu*Q / (PI*(R0*F(0, 0))*(R0*F(0, 0))*(R0*F(0, 0)));

	networkSolver(num_nodes, num_fibers, x_scale, F, network0,
		network_n, fiberResults_n, netResults_n, guess, stab_node, WSS);
	...
	double k = 200, k2 = 381, k3 = 0.241; //k gives cfcd give 60mEq/L at 30% actin, k2 is from Ateshian (2009) JBME 131
	...
	err(1) = (appliedStress(1) - net_stress(1, 1));
	err(2) = (appliedStress(2) - net_stress(2, 2));
}