...
void calcJacobian2(MatrixXd& nodes_n, MatrixXd& fibers_n, network& network_n, Matrix3d F, VectorXd& node_forces,
	int num_fibers, int num_nodes,  SparseMatrix<double>& J, VectorXi stab_node, double WSS)
{
	VectorXd init_lens = network_n.init_lens;
	VectorXd fib_areas = network_n.fib_areas;
	VectorXi fib_type = network_n.fib_type;

	int xyz_num = 3 * num_nodes;

	MatrixXd Jac = MatrixXd::Zero(xyz_num, xyz_num); //clear Dense Jacobian

	//typedef Eigen::Triplet<double> T; // triplet initiation to store sparse Jacobian
	//vector<T> J_Coefficients; // Storage of sparse Jacobian coefficients
	//J_Coefficients.reserve(xyz_num);

		// private copies of variables
		int node1, node2;

		Vector3d realNode2, old_vect, unit_old, node_force1, node_force2, dlamdx1, dlamdx2; 

		double fib_force, dFdlam, lambda, vect_len,
			dndx11, dndx12,  dndx21, dndx22, // dndx F node2 and x node2
			j_temp11,  j_temp12, j_temp21, j_temp22, inv_vect_len3; // temporary jacobian 
		...
		for (int n = 0; n < num_fibers; n++)
		{
			...
			inv_vect_len3 = 1 / (vect_len*vect_len*vect_len);

			// fiber unit vector
			unit_old = old_vect/vect_len;

			// calling fiber constitutive equation to get fib_forces and dFdlam
			fiberConstEqn(old_vect, node_force1, node_force2, lambda, init_lens(n), fib_type(n), fib_areas(n), fib_force, dFdlam, WSS);
	...
	// Constraining the stabilized node2 to their initial positions

	double max_J = 100.0*Jac.maxCoeff();
	...
	double drop_tol = 1e-18; // 1.0e-4*Jac.maxCoeff();
	/*
	if (drop_tol < 1e-16)
	{	
		drop_tol = 1e-16;
	}
	else if (drop_tol > 1e-12)
	{
		drop_tol = 1e-12;
	}
	*/

	J = Jac.sparseView(drop_tol, 1);

	/*
	// Putting Jacobian values in sparse matrix
	for (int r = 0; r < 3 * num_nodes; r++)
	{
		for (int v = 0; v < 3 * num_nodes; v++)
		{
			if (Jac(r, v) > drop_tol || Jac(r, v) < -drop_tol)
			{
				J_Coefficients.push_back(T(r, v, Jac(r, v))) ; // storing sparse coefficients
			}
		}
	}

	J.setFromTriplets(J_Coefficients.begin(), J_Coefficients.end()); // setting sparse matrix for Jacobian
	*/
}