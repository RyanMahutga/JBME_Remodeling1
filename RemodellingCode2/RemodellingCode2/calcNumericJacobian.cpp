	...
	int num_fibers, int num_nodes,  SparseMatrix<double>& J, VectorXi stab_node)
{
	...
	//J_Coefficients.reserve(xyz_num);
		...
		for (int n = 0; n < num_fibers; n++)
		{
			...
			old_vect =  realNode2 - Vector3d(nodes_n.row(node1)); // fiber span

			vect_len = old_vect.norm(); // fiber length

			inv_vect_len3 = 1.0 / (vect_len * vect_len*vect_len); // 1 / cubed vector length

			// fiber unit vector
			unit_old = old_vect/vect_len;

			// calling fiber constitutive equation to get fib_forces and dFdlam
			fiberConstEqn(old_vect, node_force1, node_force2, lambda, init_lens(n), fib_type(n), fib_areas(n), fib_force, dFdlam);
	...
	double drop_tol = 1.0e-16*Jac.maxCoeff();
	if (drop_tol < 1e-18)
	{	
		drop_tol = 1e-18;
	}
