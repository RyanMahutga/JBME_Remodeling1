...
int main()
{
	...
	inFile.open("PeriodicNetwork_299R2e.txt");
	...
	inFile2.open("PeriodicNetwork_2990R2e.txt");
	if (inFile.fail()) { // if can't open file
		cout << "Unable to Open Network File!" << endl;
		cin.get();
		exit(1); // terminate with error
	...
	{
		strfile = "OverloadRem";
		cout << "Overload Removed ... " << endl << endl;
		Shear_Given = 0.0;
	...
	for (int t = 0; t < time_steps; t++)
	{
		...
		{
			P = (P_Given - P_inf)*(t-100.0) / 100.0 + P_inf;
		}
		else
		{
			P = P_Given;
