...
{
	int check1 = 0;
	double dlam = 5e-4, mag_err = 1e6, err_thresh = 100, newflag = -1.0, cond = 0.0, sbnd = 0.1, scalef=0.25;
	Matrix3d dF, J, F_o;
	...
	for (int n = 0; n < 50; n++)
	{
		...
		solveJac(num_nodes, num_fibers, x_scale, F, network0, network_n, fiberResults_n, netResults_n,
			guess, stab_node, appliedStress, net_stress, init_err, P, fib_vol_m, WSS, Q);
		...
		for (int i = 0; i < 3; i++)
		{
				...
				solveJac(num_nodes, num_fibers, x_scale, dF, network0,
					...
					net_stress, err, P, fib_vol_m, WSS, Q);
...
//		}
		...
		*/
		//cout << "External Condition #: " << cond << endl << endl;
			...
			else if (n > 19 && n < 31) // Help get search unstuck
			{
				scalef = rand() / (2*(double(RAND_MAX)));
				F(0, 0) += scalef*epsilon(0);
				F(1, 1) += scalef *epsilon(1);
				F(2, 2) += scalef *epsilon(2);
			}
			else
			{
				F(0, 0) += epsilon(0);
