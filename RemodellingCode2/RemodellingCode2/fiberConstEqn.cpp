...
void fiberConstEqn(Vector3d vect1, Vector3d& node_force1, Vector3d& node_force2, double& lambda, double init_len,  int fibtype,double fib_area, double& fib_force, double& dFdlam, double WSS)
{
	...
	else if (fibtype == 11 || fibtype == 2 || fibtype == 13) // fiber force from F = k(GS) where GS = Green strain = 0.5(lambda^2-1) 
	{//(use 2 for elastin, 5 for failed fibers)
		...
		if (fibtype == 2)
		{
			...
			{
				fib_mod = 1e6/100.0; // 1% of elastin stiffness for failed fibers
				dFdlam = fib_mod * fib_area*lambda;
			}
		}
		else if (fibtype == 11) // actin
		{
			...
			{
				fib_mod = 4e6; //0.036e9; // 0.0011e9 see Elastic Proteins: biological roles and mechanical properties Gosline, J. et al. (2002) Phil. Trans. R. Soc. Lond.
				dFdlam = fib_mod * fib_area*lambda;
			}
			else
			{
				fib_mod = 4e6/10000.0; // 1% of elastin stiffness for failed fibers
				dFdlam = fib_mod * fib_area*lambda;
			}
		}
		else // collagen
		{
			...
			{
				fib_mod = 700e6/10000.0; // 1% of elastin stiffness for failed fibers
				dFdlam = fib_mod * fib_area*lambda;
			}
		}
	...
	else if (fibtype == 1) // actin
	{
		...
		if (lambda >= 1.0) //  give 12pN force at lam = 1.05
		{
			fib_mod = 3000e3; //0.036e9; // 0.0011e9 see Elastic Proteins: biological roles and mechanical properties Gosline, J. et al. (2002) Phil. Trans. R. Soc. Lond.
			dFdlam = fib_mod * fib_area*lambda;
		}
		else
		{
			fib_mod = 3000e3 / 100.0; // 1% of elastin stiffness for failed fibers
			dFdlam = fib_mod * fib_area*lambda;
		}
		//active contribution
		if (lambda > 0.75 && lambda < 1.25)
		{
			double S0 = 400e3, l_min = 0.75, l_max = 1.0, WSS_inf = 5.0; // estimate from Alford integrative biology //0.036e9; // 0.0011e9 see Elastic Proteins: biological roles and mechanical properties Gosline, J. et al. (2002) Phil. Trans. R. Soc. Lond.

			double fib_S = S0 * (WSS_inf / WSS);
