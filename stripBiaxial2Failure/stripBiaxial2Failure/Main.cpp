...
int main()
{
	...
	*/
	...
	cout << "Strip Biaxial to Failure: " << endl;
	...
	double lam_S = 1.4*lam0(1), shear = 0;

	string strfile = "stripbiaxial";
	...
	while (exit_flag == 0)
	{
		...
		while (num_fail > 1 && flag2==0)
		{
			...
			if (num_fail > 5 && dlam > 0.005 && k>0)
			{
				dlam = dlam - 0.001;
			}
			else if (num_fail == 0 && dlam < 0.04)
			{
				dlam = dlam + 0.01;
			}
			else
			...
			if (k > 1)
			{
				drop1 = (net_stress(0, 0) - xstress(k-1,0))/dlam;
				drop2 = (net_stress(1, 1) - ystress(k-1,0))/dlam;
				if (drop1 < -1000e3)
				{
					break_flag1 = 1;
				}
			...
			*/
			...
			if (tot_fail > (0.15 * num_fibers))
			{
				writeData(num_fibers, network_n, fiberResults_n, netResults_n, k);

				writePeriodicNet2File(network_n, num_fibers, num_nodes, stab_node, k);
			}
			if (tot_fail >  (0.5 * num_fibers) || lam > 2.5)
			{
				exit_flag = 1;
			}
		...
		xzstress(k, 0) = net_stress(0, 2) + pressure;
		...
		appendResults(stretch.row(k), xstress.row(k), ystress.row(k), zstress.row(k), xzstress.row(k), phi.row(k), shear, total_failed_types.row(k));
